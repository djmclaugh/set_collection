...
export default class UnionPowerPane extends View {
  private button: Button;
  private selection: ZFCSet|null = null;
  private selectionView: SetView|null = null;
  constructor() {
    super();
    const t = newTextField("Choose a set then press the button to unlock the set containing all the elements of the elements of the chosen set.", W - 80);
    t.x = (W - t.width) / 2;
    t.y = 80;
    this.container.addChild(t);

    this.button = new Button("Union!");
  ...
  public onClick(callback: DiscoverCallback) {
    this.button.onClick(() => {
      if (this.selection != null) {
        callback(ZFCSet.union(this.selection));
      }
    });
  }
}
